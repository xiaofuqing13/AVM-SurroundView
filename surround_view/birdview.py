...
class BirdView(BaseThread):
    ...
    def __init__(
        ...
        self.camera_ids_in_order = camera_ids_in_order
        self.drop_if_full = drop_if_full
        self.buffer = Buffer(buffer_size)
    ...
    def load_weights_and_masks(self, weights_image, masks_image):
        GMat = (
            np.asarray(Image.open(weights_image).convert("RGBA"), dtype=float) / 255.0
        )
        self.weights = [
            np.stack((GMat[:, :, k], GMat[:, :, k], GMat[:, :, k]), axis=2)
            for k in range(4)
        ]
    ...
    def run(self):
        self.running = True
        while self.running:
            images = self.proc_buffer_manager.get()
            front, back, left, right = (
                images[self.camera_ids_in_order[0]],
                images[self.camera_ids_in_order[1]],
                images[self.camera_ids_in_order[2]],
                images[self.camera_ids_in_order[3]],
            )
            self.frames = [front, back, left, right]

            if self.use_gains:
                self.make_luminance_balance()
            ...
            self.copy_car_image()
            self.buffer.add(self.image, self.drop_if_full)
