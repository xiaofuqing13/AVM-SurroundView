...
class CaptureThread(BaseThread):
    ...
    def run(self):
        if self.buffer_manager is None:
            raise ValueError(
                "This thread has not been binded to any buffer manager yet"
            )
        ...
        while True:
            ...
            self.new_frame.emit(self.device_id, frame)
    ...
    def connect_camera(self):
        if self.use_gst:
            options = gstreamer_pipeline(
                cam_id=self.device_id, flip_method=self.flip_method
            )
