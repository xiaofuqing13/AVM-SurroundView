...
class CameraProcessingThread(BaseThread):
    ...
    def run(self):
        if self.proc_buffer_manager is None:
            raise ValueError(
                "This thread has not been binded to any processing thread yet"
            )
        ...
        while True:
            ...
            image = self.capture_buffer_manager.get_device(self.device_id).get().image
            und_image = self.camera_model.undistort(image)
            proj_image = self.camera_model.project(und_image)
            flipped_image = self.camera_model.flip(proj_image)
            self.proc_buffer_manager.set_frame_for_device(self.device_id, flipped_image)
